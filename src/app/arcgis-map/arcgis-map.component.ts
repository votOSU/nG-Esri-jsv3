...
export class ArcgisMapComponent implements OnChanges
{
  ...
  changeMapLayerType(event) {
    ...
    this.createMapView(this.selectedType);
  }
  ...
  createMapView(currentSelected: string) {
    ...
    loadModules([
      ...
      .then(([
        ...
        let map = new Map(this.mapNodeElementRef.nativeElement, {
        ...
        if (currentSelected === 'id1') {
          ...
          console.log("SOMETHING ID1");
        }
        else if (currentSelected === 'id2') {
          document.getElementById("year2007").addEventListener("click", function (e) {
            map.removeAllLayers();
            layer.setVisibleLayers([2]);
            map.addLayer(layer);
            console.log("SOMETHING ID2");
          })
        }
